 ...
 * markdownLoader.js
 *
 * Loads Markdown study content files from the data directory.
 *
 * Each Markdown file represents one textbook chapter.
 * The file has a YAML frontmatter section at the top that contains metadata
 * (board, class, subject, section, chapter_no, etc.) followed by the actual content.
 *
 * MAIN EXPORTS:
 *   loadMarkdownDocuments(baseDir)     → loads all markdown files eagerly
 *   loadMarkdownDocumentsLazy(baseDir) → loads one file at a time (generator)
 *   validateMarkdownDocument(doc)      → validates metadata fields
 *   parseYamlFrontmatter(content)      → parses YAML header from file content
 */
...
export const parseYamlFrontmatter = (content) => {
  ...
  if (!normalizedContent.startsWith('---')) {
    return { metadata: {}, content: normalizedContent.trim(), hadFrontmatter: false };
  }

  const lines = normalizedContent.split(/\r?\n/);
  const endIndex = lines.findIndex((line, index) => index > 0 && line.trim() === '---');

  if (endIndex === -1) throw new Error('YAML frontmatter opening marker found without closing marker.');
  ...
  for (const line of lines.slice(1, endIndex)) {
    ...
    if (separatorIndex === -1) throw new Error(`Unsupported YAML frontmatter line: ${line}`);
    const key = trimmed.slice(0, separatorIndex).trim();
    metadata[key] = parseYamlScalar(trimmed.slice(separatorIndex + 1));
  }

  return { metadata, content: lines.slice(endIndex + 1).join('\n').trim(), hadFrontmatter: true };
};
...
const normalizeDocument = (doc, baseDir) => {
  ...
  const metadata = { ...parsed.metadata, source_path: sourcePath, file_name: fileName };
  const normalizedDoc = { id: createStableId(metadata), pageContent: parsed.content, metadata };

  const validation = validateMarkdownDocument(normalizedDoc, { baseDir, absoluteSourcePath });
  if (!validation.valid) {
    throw new Error(`Invalid markdown document metadata in ${sourcePath}:\n- ${validation.errors.join('\n- ')}`);
  }
...
export const hasNativeLangChainLazyLoad = (baseDir) => {
  const loader = new DirectoryLoader(baseDir, { '.md': (filePath) => new TextLoader(filePath) });
  return typeof loader.lazyLoad === 'function';
};

export const loadMarkdownDocuments = async (baseDir) => {
  const absoluteBaseDir = path.resolve(baseDir);
  const loader = new DirectoryLoader(absoluteBaseDir, { '.md': (filePath) => new TextLoader(filePath) });
...
export const loadMarkdownDocumentsLazy = async function* (baseDir) {
  const absoluteBaseDir = path.resolve(baseDir);
  const directoryLoader = new DirectoryLoader(absoluteBaseDir, { '.md': (filePath) => new TextLoader(filePath) });
...
export const validateMarkdownDocument = (doc, options = {}) => {
  ...
  if (!doc.pageContent || doc.pageContent.trim().length === 0) errors.push('pageContent is empty.');
  ...
  const absoluteSourcePath = options.absoluteSourcePath
    ...
    : metadata.source_path ? path.resolve(process.cwd(), metadata.source_path) : '';
