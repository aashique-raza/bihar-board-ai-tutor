 ...
 * markdownChunker.js
 *
 * Splits loaded Markdown documents into smaller chunks for vector embedding.
 *
 * CHUNKING STRATEGY:
 *   1. Parse the document into heading-based sections (## , ### , etc.)
 *   2. Merge small sections together to avoid tiny chunks
 *   3. Split large sections using LangChain's RecursiveCharacterTextSplitter
 *   4. Add rich metadata to each chunk (chapter, heading path, content type, etc.)
 *   5. Prepend a [Context] header to each chunk for better retrieval
 *
 * MAIN EXPORTS:
 *   createMarkdownChunks(documents, options) → chunked documents
 *   validateChunk(chunk)                     → validates a single chunk
 */
...
const REQUIRED_CHUNK_METADATA = [
  'board', 'class', 'subject', 'section', 'chapter_no', 'original_science_chapter_no',
  'chapter_title', 'source_path', 'file_name', 'chunk_index', 'chunk_id',
  'heading_path', 'heading_level', 'content_type', 'char_count', 'word_count',
  'line_count', 'splitter', 'chunk_size_config', 'chunk_overlap_config',
];

export const ALLOWED_CONTENT_TYPES = [
  'concept', 'definition', 'formula', 'example', 'activity',
  'question', 'table', 'diagram_reference', 'mixed',
];

const mergeConfig = (options = {}) => ({ ...DEFAULT_CHUNK_CONFIG, ...options });
...
// Each chunk starts with a [Context] header to help the LLM understand the source
...
const isOnlyHeadingText = (content) => {
  ...
  return meaningfulLines.length > 0 && meaningfulLines.every((line) => /^#{1,6}\s+\S+/.test(line));
};

const isMeaningfulRawContent = (content) => {
  const trimmed = content.trim();
  if (!trimmed || isOnlyHeadingText(trimmed)) return false;
...
const getHeadingPath = (stack, fallbackTitle) =>
  stack.length === 0 ? fallbackTitle : stack.map((heading) => heading.text).join(' > ');
...
const parseHeadingSections = (doc) => {
  ...
  for (const line of lines) {
    ...
    if (!currentSection) {
      currentSection = { headingPath: doc.metadata.chapter_title, headingLevel: 0, headingStack: [], contentLines: [] };
    }
...
const mergeSmallSections = (sections, config) => {
  ...
  const flushBuffer = () => {
    if (buffer && isMeaningfulRawContent(buffer.text)) mergedSections.push(buffer);
    buffer = null;
  };
...
const detectContentType = (headingPath, rawContent) => {
  ...
  if (/\bdefinition\b|\bmeans\b|\bis called\b|\bknown as\b/.test(text)) matchedTypes.add('definition');
  if (/\bexample\b|\bnumerical\b|\bsolution\b|\bgiven\b/.test(text)) matchedTypes.add('example');
  if (/\bactivity\b/.test(text)) matchedTypes.add('activity');
  if (/\bquestion\b|\bexercise\b|\bshort answer\b|\blong answer\b|\bmcq\b/.test(text)) matchedTypes.add('question');
  if (/^\s*\|.+\|\s*$/m.test(rawContent)) matchedTypes.add('table');
  if (/\bfigure\b|\bdiagram\b|\bshown in figure\b|\bfig\./.test(text)) matchedTypes.add('diagram_reference');
  if (/[A-Za-z0-9)]\s*=\s*[A-Za-z0-9(]/.test(rawContent) || /\b[VPIHRE]\s*=\s*[A-Za-z0-9]/.test(rawContent)) matchedTypes.add('formula');
  if (matchedTypes.size > 1) return 'mixed';
...
const splitSection = async (section, config) => {
  if (section.text.length <= config.chunkSize) return [{ text: section.text, splitter: 'heading_merge' }];
  const splitter = createSplitter(config);
  const splitTexts = await splitter.splitText(section.text);
  return splitTexts.map((text) => text.trim()).filter(isMeaningfulRawContent).map((text) => ({ text, splitter: 'recursive_character' }));
};
...
export const createMarkdownChunks = async (documents, options = {}) => {
  ...
  const chunks = [];
  for (const doc of documents) chunks.push(...(await createChunksForDocument(doc, config)));
  return chunks;
};
...
export const validateChunk = (chunk) => {
  const errors = [];
  if (!chunk.id) errors.push('Chunk id is missing.');
  if (!chunk.pageContent || chunk.pageContent.trim().length === 0) errors.push('pageContent is missing or empty.');
  if (!chunk.pageContent?.includes('[Context]')) errors.push('pageContent is missing [Context].');
  if (!chunk.pageContent?.includes('[Content]')) errors.push('pageContent is missing [Content].');
  if (chunk.pageContent?.trimStart().startsWith('---')) errors.push('pageContent starts with YAML frontmatter marker.');
  if (!chunk.metadata) errors.push('metadata is missing.');
  for (const field of REQUIRED_CHUNK_METADATA) {
    if (chunk.metadata?.[field] === undefined || chunk.metadata?.[field] === null || chunk.metadata?.[field] === '') {
      errors.push(`Missing required metadata field: ${field}`);
    }
  }
  if (chunk.metadata?.chunk_id !== chunk.id) errors.push('metadata.chunk_id does not equal chunk.id.');
  if (!(chunk.metadata?.char_count > 0)) errors.push('metadata.char_count must be greater than 0.');
  if (!(chunk.metadata?.word_count > 0)) errors.push('metadata.word_count must be greater than 0.');
  if (!chunk.metadata?.heading_path) errors.push('metadata.heading_path is missing.');
  if (!ALLOWED_CONTENT_TYPES.includes(chunk.metadata?.content_type)) errors.push(`Invalid content_type: ${chunk.metadata?.content_type}`);
  const contentPart = chunk.pageContent?.split('[Content]')[1]?.trim() || '';
  if (!isMeaningfulRawContent(contentPart)) errors.push('Chunk content is empty, heading-only, or context-only.');
  if (chunk.pageContent?.length > (chunk.metadata?.chunk_size_config || DEFAULT_CHUNK_CONFIG.chunkSize) + 1300) errors.push('Chunk is far above configured chunk size.');
  if (chunk.pageContent?.length > 2500) errors.push('Chunk exceeds 2500 characters.');
