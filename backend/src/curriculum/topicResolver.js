 ...
 * topicResolver.js
 *
 * Resolves a free-text message to a specific topic within a chapter.
 * Uses fuzzy token matching and heading path search.
 *
...
const getSearchableTopicText = (topic) => [topic.title, topic.headingPath, ...(topic.ragHints || [])].join(' ');
...
const createTopicMatch = ({ topic, normalizedText, chapterScoped }) => {
  ...
  let score = 0;
  const reasons = [];
  if (titleText && normalizedText.includes(titleText)) { score += 100; reasons.push('topic_title'); }
  if (headingPathText && normalizedText.includes(headingPathText)) { score += 60; reasons.push('heading_path'); }
  const titleOverlap = countTokenOverlap(messageTokens, titleTokens);
  if (titleOverlap > 0) { score += titleOverlap * 25; reasons.push('topic_title_tokens'); }
  const topicOverlap = countTokenOverlap(messageTokens, topicTokens);
  if (topicOverlap > titleOverlap) { score += (topicOverlap - titleOverlap) * 8; reasons.push('topic_hint_tokens'); }
  if (chapterScoped) { score += 15; reasons.push('chapter_scope'); }
  if (topic.role === 'core') score += 5;
  else if (topic.role === 'subtopic') score += 4;
  else if (topic.role === 'overview') score += 2;
  else if (topic.role === 'practice') score -= 10;
...
const buildResult = ({ status, match = null, matches = [], chapter = null, reason }) => ({
  ...
  chapter,
  matches: matches.map((item) => ({ topic: toPublicTopic(item.topic), score: item.score, reasons: item.reasons })),
  reason,
});
...
export const resolveTopic = (curriculumIndex, message, options = {}) => {
  ...
  const chapterId = chapterMatch.status === 'resolved' && chapterMatch.chapter ? chapterMatch.chapter.chapterId : null;
  const chapterScoped = Boolean(chapterId);
  const topics = flattenTopics(curriculumIndex, chapterId);

  if (!normalizedText) return buildResult({ status: 'not_found', chapter: chapterMatch.chapter || null, reason: 'Empty message.' });
  ...
  if (matches.length === 0) return buildResult({ status: 'not_found', chapter: chapterMatch.chapter || null, reason: 'No topic matched the message.' });

  const bestScore = matches[0].score;
  const topMatches = matches.filter((match) => match.score === bestScore);

  if (topMatches.length > 1) return buildResult({ status: 'ambiguous', matches: topMatches, chapter: chapterMatch.chapter || null, reason: 'Multiple topics matched equally well.' });

  return buildResult({ status: 'resolved', match: matches[0], matches: matches.slice(0, 5), chapter: chapterMatch.chapter || null, reason: 'Best topic match selected.' });
};
