...
const tutorResponsePrompt = ChatPromptTemplate.fromMessages([
  [
    'system',
...
export const generateTutorResponse = async ({
  ...
  const rawResponse = await getResponseChain().invoke({
    message,
    answerLanguageInstruction: getAnswerLanguageInstruction(answerLanguage),
    responseMode,
    decision,
    memory,
    history,
  ...
  const normalized = {
    ...
    sections: sections.length ? sections : createFallbackResponse({ responseMode, message }).sections,
