...
| **Sub-stage** | **Stage G (review + final health) — IN PROGRESS, paused mid-phase.** `npm run quiz:review` chal chuka hai. Red queue **0**. **L3+ answer-confidence 75.1% (need ≥90%)** — root cause investigated this session (3 distinct causes found, 1 fixed, 2 remain). |
...
| **Last session** | 2026-08-06 (session 2) — Investigated the L3+ gap (73.1%→need ≥90%) instead of guessing. Found **3 distinct causes** in the 205 unverified objective MCQs: (1) ~16 extraction bugs — `2018-b` has the printed answer baked into the question stem (e.g. `"...Ans: concave mirror"`), `2019-b:A:1` is the exam's own instructions line mis-parsed as a question — **not fixed this session, still open**. (2) ~53 cache entries with verdict `ungrounded` — a **real bug, fixed**: `buildAnswers.js`'s `evidenceIsGrounded()` required a literal substring match, but source excerpts are markdown bullet/numbered lists (e.g. `"- Right atrium\n- Right ventricle"`) which a model naturally quotes as flattened prose (`"Right atrium, Right ventricle"`) — same words, different punctuation, so a correct, well-evidenced answer was rejected as "ungrounded". Fixed `normalizeForMatch()` to strip list markers/punctuation before comparing (still requires the same words in the same order — doesn't loosen the hallucination check itself). (3) ~137 "insufficient" verdicts — mixed: some are genuine retrieval misses (confirmed directly against source: e.g. "hardest natural substance" → the answer is a table row in `chapter-03-metals-and-non-metals.md` that never made it into the top-8 retrieved chunks), some are genuine content gaps (e.g. "1 HP is equal to" — literally not in `data/class-10/science/`, correctly rejected) — **not fixed this session, still open**. **Self-caught bug in my own fix:** first attempt blindly promoted cached `ungrounded` verdicts to `answered` using their old `claimedOption` — direct inspection of `2023-a:A:30` (heart chambers) showed this had silently promoted a **wrong answer** (option "a"=1, when correct is "d"=4) — the model's original quote was real text but had been mismapped to the wrong option letter, something the grounding check never verified. Reverted before running the full bank, fixed properly: stale `ungrounded` cache entries now always get a fresh LLM re-verification (not a free promotion), confirmed `2023-a:A:30` now correctly resolves to "d". Re-ran Stage E→F→G on all 18 papers. **L3+: 73.1%→75.1%** (552→567 usable). Baseline clean before and after, no regression. **Agla:** two causes remain — (1) fix the ~16 extraction bugs (2018-b, 2019-b:A:1), (2) investigate the retrieval-miss cases in the 137 "insufficient" bucket (may need reranker/chunking work, separate from this pipeline's scripts). Once both move, re-check full §12 exit criteria + golden-set 30/30 (not re-checked this session). |
