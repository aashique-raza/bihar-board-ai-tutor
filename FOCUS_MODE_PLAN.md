...
### Phase I — Deterministic topic→chunk linking (fixes the NEXT_STEP duplicate-content class of bug for good) — DONE 2026-07-21

**Why this happened:** BUG-6's regex fix (Phase H) couldn't be live-verified end-to-end in that session (environment issue). Before re-verifying, a deeper question was asked: is the underlying retrieval mechanism ("semantically guess the right chunk for a topic") reliable at all, or was BUG-6 just one symptom of it? A full audit — [`RETRIEVAL_TOPIC_LINKING_PLAN.md`](RETRIEVAL_TOPIC_LINKING_PLAN.md) — answered that with evidence: 87 chunks across all 16 chapters genuinely span more than one core topic, so any topic→content lookup based on semantic guessing was structurally unreliable, not just occasionally unlucky.

**The fix:** at chunk-creation time, every chunk now records exactly which curriculum topic(s) it belongs to (`metadata.topic_ids`), computed by walking the heading structure — not guessed. `NEXT_STEP` retrieval switched from "semantic search for a topic" to "exact fetch of the chunk(s) already known to belong to this topic" (`retrieveChunksByTopicId()` in `retriever.js`). Required a manual one-time Atlas Console change (adding `metadata.topic_ids` as a filterable field) — done and confirmed working mid-audit, 2026-07-12.

**A second bug found via this same audit, before the fix's own code was written:** documented separately in [`ROLE_CLASSIFICATION_AUDIT.md`](ROLE_CLASSIFICATION_AUDIT.md) — `getTopicRole()` classified a heading as `revision` based on keywords found anywhere in its *full ancestry path*, not just its own title. This wrongly excluded real teaching content (worst case: Light chapter's entire "Ray Rules / Image Formation / Uses of Mirrors" subtree, 3249 characters) from any core topic — starving that topic and causing exactly the kind of duplicate/wrong content NEXT_STEP had already been seen to produce. Two cheaper options (rename the heading; loosen the classifier) were evaluated and rejected (one was proven non-cosmetic via simulation, the other risked pulling exam questions into teaching content) in favor of **Option D**: when linking a chunk to its owning topic, climb past any non-core ancestor to find the nearest real `core` ancestor above it, instead of stopping at the first revision/practice node. Zero regressions verified by simulation before implementing. **This was folded directly into the same `markdownChunker.js` change as the main linking fix — not built as a separate pass.**

**What happened across the 9-day gap between designing this and closing it out:** the code for both fixes was fully written (matches both plan docs line-for-line) but sat **uncommitted** and with **no record of whether the one-time DB migration script had ever been run**. On resuming, this was resolved cleanly instead of guessed: the (read-only, zero-risk) coverage-verification script was run *first*, specifically to answer "was the migration already done?" — it confirmed real linked data existed, so yes. The migration script was then re-run anyway (to pick up an uncommitted manual content edit to the Chemical Reactions chapter from the same gap), confirmed 629/629 chunks updated with zero drift, then the coverage check was re-run and still passed.

**Live-verified (2026-07-21):** Farhan ran a real Focus Mode session — "Periodic Classification of Elements" (not one of the two chapters this plan originally scoped for live testing, but one flagged by the coverage script's "shared, not exclusive, content" warning list). Most topics advanced with genuinely distinct content. One topic (`topic-16`, "Mendeleev's Periodic Table") showed a real, visible partial repeat — confirming the warning list is a real predictor of user-visible risk. Decision: accept this as the same pre-existing, documented "merge-bleed" limitation already scoped as out-of-fix in the original plan (Part 5) — not a regression, not launch-blocking.

**Explicitly NOT done, flagged rather than assumed:** the two chapters that originally surfaced the headline bugs (Light — Option D's severe case; Chemical Reactions — BUG-6) were not live re-tested this session. Indirect evidence is favorable (Light's severe topic no longer appears in the coverage warning list) but not confirmed by an actual conversation. Recommended next-session action if this needs closing further: a quick "Aage badhao" x4 run on Chemical Reactions specifically.

**Files changed:** `backend/src/rag/markdownChunker.js` (topic_ids linking + Option D logic, combined), `backend/src/ask/step5.retrieveContent.js` (NEXT_STEP switched to deterministic fetch), `backend/src/rag/retriever.js` (new `retrieveChunksByTopicId()`), `backend/scripts/backfill-chunk-topic-ids.js` (new), `backend/scripts/verify-topic-chunk-coverage.js` (new, includes the Option D-specific "no exclusive content" check), `data/class-10/science/chemistry/chapter-01-chemical-reactions-and-equations.md` (minor content addition, did not achieve its intent — see plan doc Part 6).

**Committed:** `6d9fb57` ("importanta bug fix in focus mode"). Not yet pushed to `origin/main`.

---

### Phase J — Exam Marks feature: hierarchy-conflation answer bug (TASK-025) — DONE 2026-07-22

**Correction to the record first:** this plan previously stated TASK-025 (Exam Knowledge Layer) had "zero code written." That was wrong — checked only `git status` (uncommitted changes) rather than the actual filesystem, and missed that the feature was already fully built and committed in an earlier session (`c0fdecf "knowledge increased"`): `examKnowledgeService.js`, `exam_patterns.json`, the `EXAM_INFO` intent, its prompt, and its `intentRouter.js` wiring all already existed and worked. Lesson: `git status` only proves what changed recently, never what exists — file-system truth needs checking directly, not inferred from diffs.

**The real bug, found via live testing:** asking "Physics kitne marks ka exam hota hai" produced a self-contradictory answer — *"Physics ka exam 100 marks ka hota hai... Physics ke liye 27 marks hain"* — because the tutor LLM was handed the entire exam-data dump (Science's totals AND Physics's sub-share, at two different hierarchy levels) and asked to compose an answer freely, and it blended the two levels. Confirmed this generalizes far beyond Physics: Social Science has 5 branches, Mathematics has 7 units, every Science chapter has its own marks — any question about a sub-component of a subject was at risk of the same conflation.

**Data was also independently found to be wrong in places** — before fixing the prompt logic, Farhan supplied 4 official BSEB syllabus PDFs (Social Science, Science, Mathematics, Hindi), which were read and cross-checked against the existing `exam_patterns.json` and two AI-compiled secondary-source reports. Found and corrected: Science's Chemistry/Biology theory-marks were swapped (official: Physics 27 / Chemistry 27 / Biology 26 — not Chemistry 26 / Biology 27 as previously recorded); Social Science's Political Science (17→18) and Economics (17→16) were off by one each. Maths and the rest of Social Science were already correct — confirmed, not assumed. Hindi's official breakdown didn't internally reconcile (105≠100) — left as the pre-existing data, not guessed at. Grading-scale data was dropped entirely (not needed, never made it into the live JSON anyway).

**Fix — same "don't trust the LLM to compose facts across a hierarchy" principle as the NEXT_STEP topic-linking fix (Phase I), applied to a different data shape:** the decider (already an LLM call, already extracts `searchQuery` for other intents) now also extracts `examEntity` — the specific subject/branch/chapter/unit named in the question, or `null` for general questions. A new, fully data-driven function (`resolveExamEntity()` in `examKnowledgeService.js`) walks the whole `exam_patterns.json` tree once (no subject names hardcoded — Social Science's branches, Maths's units, and every Science chapter are covered automatically) and returns the exact own-marks + parent-paper relationship for any resolved name. That gets formatted into a fixed `### DIRECT ANSWER FACT ###` block prepended to the context; `examInfoPrompt.js` was told to repeat those numbers exactly, not recompute them. Unresolved/general questions fall back to the original full-context-dump behavior unchanged — no regression risk there.

**Files changed:** `backend/src/knowledge/examKnowledgeService.js` (new `resolveExamEntity()`/`formatEntityFact()`), `backend/src/prompts/deciderPrompt.js` (examEntity extraction rules + schema), `backend/src/ask/step4.decideRetrieval.js` (passes examEntity through), `backend/src/ask/step5.retrieveContent.js` (resolves + prepends the fact block), `backend/src/prompts/intents/examInfoPrompt.js` (trust-the-fact-block instruction), `data/class-10/global/exam_patterns.json` (data corrections above).

**Verified (2026-07-22), live end-to-end against a real backend:** "Physics kitne marks ka?" → correct, no self-contradiction, `examEntity: "Physics"` resolved. "History kitne marks ka?" (Social Science branch) → correct. "Life Processes se kitne marks?" (chapter-level) → correct. "Pass karne ke liye kitne marks chahiye?" (general) → `examEntity: null`, fell back to the original behavior correctly, no regression. Committed `466170d`.

---

### Phase K — Physics/Biology chapter mis-categorization ("Our Environment" + "Management of Natural Resources") — DONE 2026-07-22

**How this was found:** while spot-checking Phase J's fix, `test:study-map` showed Physics with 7 chapters and Biology with only 4 — should be 5 and 6. Farhan independently confirmed via two web searches (screenshots) that Biology has 6 chapters including "Our Environment" and "Management of Natural Resources," which the project's own files had filed under `data/class-10/science/physics/` as chapter_no 6/7, with `section: Physics` in their frontmatter — since this project's earliest content pass. The official Science.pdf synced with Farhan earlier (Phase J's data work) independently confirmed these two belong under "जीव विज्ञान" (Biology).

**Full blast-radius audit done before touching anything** (explicitly requested — this project's history includes fixing one thing and breaking another, so this one was audited file-by-file, schema-by-schema, with live DB queries, before any edit):
- `curriculumIndexBuilder.js` — confirmed fully data-driven (chapterId/topicId are computed from `metadata.section`+`chapter_no`, nothing hardcoded) — zero code changes needed there.
- `chapterResolver.js`/`nextTopicResolver.js`/`topicResolver.js` — same, fully data-driven; `chapterResolver.js` is also test-only per its own header comment, not live runtime code.
- Frontend — zero hardcoded chapter/section references (grepped `frontend/src` directly).
- Live MongoDB queried directly (not assumed): **zero** `ChapterProgress`/`ChatSession`/`StudyEvent` documents referenced these 2 chapters — nobody, ever, had studied them via Focus Mode. This eliminated the biggest feared risk (broken student progress) before writing a line of code.
- `markdownLoader.js`'s `SECTION_RULES` validation *had baked the mistake in as if it were correct* (`physics: chapterEnd: 7`) — confirming the folder-placement mistake and the validation-rule mistake were made together, long ago. Fixing this also surfaced a real formula limitation: Biology's original NCERT numbering is **not contiguous** once these 2 chapters join it (1-4 → original 6-9, but 5-6 → original 15-16, with 10-14 belonging to Physics in between) — the old `originalStart + chapterNo - 1` linear formula can't express that gap.
- 60 `Chunk` documents in MongoDB needed relabeling (`metadata.section`/`chapter_no`/`chunk_id`, and the `[Context]` header baked into `pageContent`) — confirmed this does **not** require re-embedding, since the embedded text (`originalText`) never includes section/chapter metadata, only `pageContent` (shown to the LLM, not embedded) does.

**Fix:** moved+renumbered the 2 files into `data/class-10/science/biology/` (chapter_no 5/6, `section: Biology`, `original_science_chapter_no` unchanged at 15/16); `markdownLoader.js`'s `SECTION_RULES` updated (Physics chapterEnd 7→5, Biology chapterEnd 4→6) with an explicit `originalMap` for Biology replacing the linear formula (Chemistry/Physics remain linear, still contiguous); `chapterHinglish.js` entries moved to the correct comment group (cosmetic only); `curriculum:build` re-run; a new one-time migration script (`backend/scripts/fix-environment-chapters-section.js`) relabeled the 60 chunks with no re-embedding; `backfill-chunk-topic-ids.js` + `verify-topic-chunk-coverage.js` re-run to resync topic links.

**A real bug caught mid-implementation, not glossed over:** the migration script's first pass updated `metadata.chunk_id` but missed a **separate, top-level `chunk_id` field** on the `Chunk` schema (required + unique — a different field from `metadata.chunk_id`, a `Mixed` sub-key). The topic-linking backfill's own drift-detection (built during Phase I) caught this immediately — 60 "chunk_id not found" + 60 "stale chunk_id" warnings, netting to zero actual data loss. A small corrective script synced the top-level field, then the backfill was re-run clean (629/629, "No drift").

**Verified (2026-07-22):** `verify-topic-chunk-coverage.js` — PASSED, same accepted 61-topic merge-bleed warning list as before Phase I/K, now correctly showing `science.biology.chapter-05`/`chapter-06` instead of the old `science.physics.chapter-06`/`chapter-07`. `test:study-map` — **Physics: 5, Chemistry: 5, Biology: 6**, confirmed correct. `test:curriculum-resolvers` — pass, unaffected. Live browser test (Farhan): Biology now shows 6 chapters including both moved ones, both teach normally via NEXT_STEP; Physics now shows exactly 5, no trace of the 2 misplaced chapters; an untouched Physics chapter (Electricity) still advances normally — no regression. No Gemini API cost incurred (metadata-only migration, matching Phase I's established pattern). Not yet committed — pending Farhan's own commit.
...
| CONCEPT_QUESTION disambiguation — student's free-text question can be genuinely ambiguous between 2+ topics in the same chapter with overlapping titles | Found during the NEXT_STEP retrieval-collision audit, 2026-07-12 | Not confirmed as a live bug yet (structural risk only), not fixed by the NEXT_STEP fix | Different mechanism from the NEXT_STEP bug — no predetermined topic ID to link to, so deterministic topic→chunk linking doesn't apply. Full detail, real example, and code path in [`CONCEPT_QUESTION_DISAMBIGUATION.md`](CONCEPT_QUESTION_DISAMBIGUATION.md). Needs its own Deep Discussion phase before any fix is designed. |
...
| BUG-6 (decimal sub-topic query mangled → duplicate content) | `[x]` | 2026-07-10 | 2026-07-10 | Root-caused via real account data (`study_events`/`chat_history`). Regex fix verified standalone; live A/B re-verification blocked by an environment issue (19 stray node processes) — see Phase H for details and the recommended follow-up check. Superseded in practice by Phase I (BUG-6's class of bug is structurally closed by the deterministic linking fix, not just this regex patch). |
| Deterministic topic→chunk linking (structural fix for the NEXT_STEP duplicate-content bug class) | `[x]` | 2026-07-12 (designed) | 2026-07-21 (implemented, backfilled, verified) | Combines `RETRIEVAL_TOPIC_LINKING_PLAN.md` + `ROLE_CLASSIFICATION_AUDIT.md`'s Option D. See Phase I above. One accepted residual (Mendeleev's Periodic Table merge-bleed); Light and Chemical Reactions chapters not live re-tested this session (flagged, not assumed fixed). |
| Exam Marks hierarchy-conflation bug (TASK-025) | `[x]` | 2026-07-22 | 2026-07-22 | Data reconciled against 4 official BSEB PDFs; decider-extracted `examEntity` + code-computed fact block replaces free-form LLM composition. See Phase J above. |
| Physics/Biology chapter mis-categorization | `[x]` | 2026-07-22 | 2026-07-22 | "Our Environment"/"Management of Natural Resources" moved from Physics (wrong, chapter 6/7) to Biology (correct, chapter 5/6). Full blast-radius audit done first; a real mid-implementation bug (top-level `chunk_id` field missed) was caught and fixed via the topic-linking backfill's own drift detection. See Phase K above. Not yet committed. |

---

## NEXT ACTION

STEP-15, the `SUBJECT_ORDER`/`CHAPTER_HINGLISH` duplication cleanup, BUG-4 (`EXPLAIN_MORE` cross-chapter leak), BUG-5 (chip loop trap), Phase I (deterministic topic-linking + Option D), Phase J (exam marks hierarchy-conflation fix), and Phase K (Physics/Biology chapter re-categorization) are all done. Phases J and K were live-verified by Farhan on 2026-07-22; Phase K is not yet committed (pending Farhan's own commit — do not commit on his behalf unless asked). Recommended before considering Phase I fully closed: a quick live "Aage badhao" x4 check on the Chemical Reactions chapter specifically (the one Phase I chapter not live re-tested). Remaining open items: STEP-13 (learningMode state machine) and the Chapter-Complete Quiz both need their own deep-discussion phase before any code is touched. The multi-subject content pipeline gap (7+ hardcoded paths) is deferred — no non-Science content exists yet to justify building it. Nothing left is deploy-blocking.
