...
STEP-15, the `SUBJECT_ORDER`/`CHAPTER_HINGLISH` duplication cleanup, BUG-4 (`EXPLAIN_MORE` cross-chapter leak), BUG-5 (chip loop trap), and Phase I (deterministic topic-linking + Option D) are all done and committed (`6d9fb57`, not yet pushed). Recommended before considering Phase I fully closed: a quick live "Aage badhao" x4 check on the Chemical Reactions chapter specifically (the one Phase I chapter not live re-tested). Remaining open items: STEP-13 (learningMode state machine) and the Chapter-Complete Quiz both need their own deep-discussion phase before any code is touched. TASK-025 (Exam Knowledge Layer) is a fully-designed, ready-to-implement plan with **zero code written yet** — a separate feature track, not a bug fix. The multi-subject content pipeline gap (7+ hardcoded paths) is deferred — no non-Science content exists yet to justify building it. Nothing left is deploy-blocking.
